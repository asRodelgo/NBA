...
tsne_teamPredicted_facets <- reactive({
  ...
  if (!(input$team_tsne_color == "K_means")) {
    ggplot(data=tsne_ready2, aes(x,y)) +  
      ...
      geom_point(data=tsne_ready2,color = eval(parse(text=input$team_tsne_color)),aes(size=eval(parse(text = size_balloons)))) +
...
thisModel <- reactive({
  ...
  datatable(model$results,
            ...
            options = list(dom='t',
                           ...
                           scroller = TRUE,
                           initComplete = JS("function(settings, json) {","$(this.api().table().header()).css({'background-color': '#1f8e2e', 'color': '#fff', 'font-size': '9px'});","}")), 
            rownames = FALSE) %>%
...
tradeUI <- function(id) {
  ...
  fluidPage(
    column(3,
           selectInput(ns('tradeTeamA'), 'Team A', choices=teamStats$Tm,selectize=FALSE), 
           br(),
           ...
           checkboxGroupInput(ns('tradePlayersA'), label = "Roster. Check players to trade", 
                              choices = filter(playerDashboard, Tm == teamStats$Tm[1])$Player, 
                              selected = NULL)
    ...
    #                           selected = NULL)
    # ),
    # column(4, DT::dataTableOutput('table_afterTrade'))
  )
}
...
tradeServer <- function(input, output, session) {
  ...
  values <- reactiveValues(playersDatabase = playersPredictedStats_adjMin)
  
  observeEvent(input$tradeButton, {
    
    values$playersDatabase <- filter(values$playersDatabase, !(Player %in% input$tradePlayersA))
    values$playersDatabase <- .redistributeMinutes(values$playersDatabase, topHeavy = 7, topMinShare = .6, min_share_top1 = .105)
    values$playersDatabase <- mutate(values$playersDatabase, 
  ...
  output$table_afterTrade <- renderDataTable({
    ...
    data <- filter(values$playersDatabase, Tm == input$tradeTeamA) %>%
      select(Player,Tm,effMin)
