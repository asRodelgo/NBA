...
observeEvent(input$tradeButton, {
  ...
  isolate({
    # if traded to a team, select it, else, target team is NULL
    if (input$tradeWhere %in% teamDashboard$Tm) targetTeam <- input$tradeWhere else targetTeam <- NULL
    
    values$playersDatabase <- .trade_Players(values$playersDatabase,Selected_Player(),tmA=NULL, playB=NULL, tmB=targetTeam) %>%
      arrange(Player)
