...
observeEvent(input$tradeButton, {
  ...
  isolate({
    # if traded to a team, assign to targetTeam variable, else, target team is NULL
    if (input$tradeWhere %in% teamDashboard$Tm) targetTeam <- input$tradeWhere else targetTeam <- NULL
    ...
    if (input$tradeWhere == "Adjust minutes") { # if only change in minutes of play
      ...
      values$playersAdjMin <- .trade_Players(values$playersAdjMin,Selected_Player(),tmA=NULL, playB=NULL, tmB=targetTeam) %>% arrange(Player)
    }
    ...
    source("code_chunks/source_computeOffenseDefense.R",local=TRUE)
    playersPredicted2 <- merge(playersPredicted,values$playersAdjMin[,c("Player","Exp","Age","Tm","effMin")], by ="Player") %>%
      mutate(adjPlusMinus = plusMinus*effMin*100) %>%
      group_by(Tm) %>%
      mutate(teamPlusMinus = sum(adjPlusMinus,na.rm=TRUE)) %>%
      ungroup()
