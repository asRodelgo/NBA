...
barplotPlayerStats <- reactive({
  ...
  #ranks <- filter(playerRanks, Player == "LeBron James") %>% 
    select(contains("eff"), contains("Per")) %>%
    gather(Stat, Value) %>%
    mutate(color = ifelse(grepl("TOV|PF",Stat),ifelse(Value > nrow(playerDashboard)/2,"green","red"),
                          ifelse(Value < nrow(playerDashboard)/2,"green","red")))
  
  data <- merge(data,ranks[,c("Stat","color")],by="Stat")
  ...
  ggplot(NULL) +
    geom_bar(data = data, aes(Stat, Value,fill=color), stat = "identity") +
...
renderValueBox({
  ...
  valueBox(
    ...
    icon = "",
    color = ifelse(rOff() < nrow(playerRanks)/2,"green","red")
  )
})
```
...
renderValueBox({
  ...
  valueBox(
    ...
    icon = "",
    color = ifelse(rDef() < nrow(playerRanks)/2,"green","red")
  )
})
```
...
renderValueBox({
  ...
  valueBox(
    ...
    icon = "",
    color = ifelse(rMin() < nrow(playerRanks)/2,"green","red")
  )
})
```
