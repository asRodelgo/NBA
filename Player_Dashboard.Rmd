...
averagePlayer <- .calculate_AvgPlayer(playersPredictedStats_adjMin) %>% mutate(Exp = as.character(floor(avg_Exp))) %>%
  ...
  as.data.frame()
# Add average player to the list of all players and assign to Team = X
#playersPredictedStats_adjMin <- rbind(playersPredictedStats_adjMin,averagePlayer)
############################
# Team loads
...
output$playerLogo <- renderImage({
  filename <- normalizePath(file.path('./images',
                              paste(tolower(gsub(" ","-",input$playerName)), '.svg', sep='')))
...
similarTeams <- reactive({
    ...
    # Euclidean distance of teams' centroids
    # distCouPerX <- filter(values$teamsTSNE,Tm == input$teamCode)$x
    # #distCouPerX <- filter(values$teamsTSNE,Tm == "CLE")$x
    # #distCouPerY <- filter(values$teamsTSNE,Tm == "CLE")$y
    # distCouPerY <- filter(values$teamsTSNE,Tm == input$teamCode)$y
    # 
    # similar_teams <- values$teamsTSNE %>%
  ...
  # similar_teams <- data.frame()
  # i <- 1
  # for (t in values$teamsDatabase$Tm) {
  #   similar_teams$Tm[i] <- t
  #   similar_teams$BC_dist[i] <- BC_distance(values$playersTSNE,values$playersDatabase,input$teamCode,t,input$num_clusters,useEffMin)
  # }
...
cluster_color_team <- reactive({
  ...
  if (!(input$team_tsne_color == "K_means")) {
    ...
    ggplot(data=tsne_ready2, aes(x,y,color = as.factor(cluster))) +
      #geom_point(data=tsne_ready2,aes(color = as.factor(cluster)),alpha = 0.2) +
      geom_point(data=tsne_ready2,aes(size=eval(parse(text = size_balloons))))+
      #geom_text(data=tsne_ready2,aes(label = paste0(Player," (",Season,")")),size=3, nudge_y = 1) +
...
# values <- reactiveValues(playersDatabase = NULL, playersRanks = NULL,showTradeText = FALSE)
# 
# values$playersDatabase <- playerDashboard
# values$playersRanks <- playerRanks
...
observeEvent(input$tradeButton, {
  ...
  isolate({
    ...
    if (input$tradeWhere == "Adjust minutes") { # if only change in minutes of play
      ...
      values$playersAdjMin <- .trade_Players(values$playersAdjMin,Selected_Player(),tmA=NULL, playB=targetPlayers, tmB=targetTeam) %>% arrange(Player)
    }
    
    # update players and teams resulting from trade
    playersPredictedStats_adjMin2 <- .redistributeMinutes(values$playersAdjMin, topHeavy = 7, topMinShare = .6, min_share_top1 = .105)
    ...
    ### Here's where average player is lost
    print(filter(values$playersDatabase, Player == "Average Player"))
    ...
    # update t_SNEs
    values$playersTSNE <- .compute_tSNE_ready_players(data = values$playersAdjMin)
    values$teamsTSNE <- .compute_tSNE_ready_teams(data = playersPredictedStats_adjPer)
...
fluidPage(
  fluidRow(
    column(2,textInput("create_name","Player name",width = '80%')),
    column(2,selectizeInput("create_position","Player position",choices = unique(playerDashboard$Pos),selected = NULL,width = '50%')),
    column(2,selectizeInput("create_team","Team",choices = unique(playerDashboard$Tm),selected = NULL,width = '50%')),
    column(2,selectizeInput("create_exp","Experience",choices = unique(playerDashboard$Exp),selected = "R",width = '50%')),
    column(2, sliderInput("create_age","Age",min = playerMin$Age, max = playerMax$Age,value = averagePlayer$Age)),
    column(2, sliderInput("create_min","Usage",min = playerMin$effMin, max = playerMax$effMin,value = averagePlayer$effMin))
  ),
  fluidRow(
    column(3, sliderInput("create_2PA","2 Point Attempt",min = playerMin$eff2PA, max = playerMax$eff2PA,value = averagePlayer$eff2PA)),
    column(3, sliderInput("create_2PM","2 Point Made",min = playerMin$eff2PM, max = playerMax$eff2PM,value = averagePlayer$eff2PM)),
    column(3, sliderInput("create_3PA","3 Point Attempt",min = playerMin$eff3PA, max = playerMax$eff3PA,value = averagePlayer$eff3PA)),
    column(3, sliderInput("create_3PM","3 Point Made",min = playerMin$eff3PM, max = playerMax$eff3PM,value = averagePlayer$eff3PM))
  ),
  fluidRow(
    column(3, sliderInput("create_FTA","FT Point Attempt",min = playerMin$effFTA, max = playerMax$effFTA,value = averagePlayer$effFTA)),
    column(3, sliderInput("create_FTM","FT Point Made",min = playerMin$effFTM, max = playerMax$effFTM,value = averagePlayer$effFTM)),
    column(3, sliderInput("create_ORB","Offensive Rebounds",min = playerMin$effORB, max = playerMax$effORB,value = averagePlayer$effORB)),
    column(3, sliderInput("create_DRB","Defensive Rebounds",min = playerMin$effDRB, max = playerMax$effDRB,value = averagePlayer$effDRB))
  ),
  fluidRow(
    column(3, sliderInput("create_AST","Assists",min = playerMin$effAST, max = playerMax$effAST,value = averagePlayer$effAST)),
    column(3, sliderInput("create_BLK","Blocks",min = playerMin$effBLK, max = playerMax$effBLK,value = averagePlayer$effBLK)),
    column(3, sliderInput("create_TOV","Turnovers",min = playerMin$effTOV, max = playerMax$effTOV,value = averagePlayer$effTOV)),
    column(3, sliderInput("create_PF","Personal Fouls",min = playerMin$effPF, max = playerMax$effPF,value = averagePlayer$effPF))
  ),
  fluidRow(
...
observeEvent(input$createPlayerButton,{
  ...
  isolate({
    ...
    newPlayer <- data.frame(Player = input$create_name, Pos = input$create_position, Season = thisSeason, Age = input$create_age, Tm = input$create$Tm, Exp = input$create_exp, FGPer= (input$create_2PM+input$create_3PM)/(input$create_2PA+input$create_3PA), FG3Per=input$create_3PM/input$create_3PA, FG2Per=input$create_2PM/input$create_2PA, effFGPer=(2*input$create_2PM+3*input$create_3PM)/(2*input$create_2PA+3*input$create_3PA),FTPer=input$create_FTM/input$create_FTA,effMin=input$create_min,effFG=input$create_2PM+input$create_3PM,effFGA=input$create_2PA+input$create_3PA,eff3PM=input$create_3PM,eff3PA=input$create_3PA,eff2PM=input$create_2PM,eff2PA=input$create_2PA,effFTM=input$create_FTM,effFTA=input$create_FTA,effORB=input$create_ORB,effDRB=input$create_DRB,effTRB=input$create_ORB+input$create_DRB,effAST=input$create_AST,effSTL=input$create_STL,effBLK=input$create_BLK,effTOV=input$create_TOV,effPF=input$create_PF,effPTS=2*input$create_2PM+3*input$create_3PM+input$create_FTM,Pick=0)
