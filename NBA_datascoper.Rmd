...
output$click_info <- renderUI({
  ...
  style <- paste0("position:absolute; z-index:100; background-color: rgba(245, 245, 245, 0.85); ",
                  "left:", left_px + 2, "px; top:", top_px + 2, "px;")
  # actual tooltip created as wellPanel
  panel_input <- paste0('Closest 10 Players (Eucl. dist.) to ',filter(data_tsne_sample, Player==tableTop10$Player[1])$Player,' in ',tableTop10$Season[1],'</a><br/><br/>')
  for (i in 2:11){
    panel_input <- paste0(panel_input, filter(data_tsne_sample, Player==tableTop10$Player[i])$Player,' (',tableTop10$Season[i],')</a><br/><br/>',round(tableTop10$dist[i],3),'<br/>')
  }
...
# ui
library(shinyBS)
tableOutput('tableBrushed')

# server
output$tableBrushed <- shiny::renderDataTable({
  brush <- input$plot_brush
